...
const TrackingPage: React.FC = () => {
    ...
    if (!order || !order.trackingHistory) {
        return (
            <div className="text-center py-10">
                <h2 className="text-2xl font-bold">Tracking information not available</h2>
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <header className="bg-white rounded-lg shadow-sm p-4 text-center">
                <h1 className="text-xl font-semibold text-gray-800">
                    Estimated Delivery By {order.estimatedDelivery}
                </h1>
            </header>

            <main className="bg-white rounded-lg shadow-sm p-6 space-y-8">
                {/* Courier Info */}
                <section className="flex items-center gap-4 border-b pb-6">
                    ...
                    <div className="flex-1 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p className="font-medium text-gray-800">Delivery Partner: {order.deliveryPartner}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Tracking Number</p>
                            <div className="flex items-center gap-2">
                                <p className="font-medium text-cyan-600">DEX-BDN-{orderId}</p>
                                <button onClick={handleCopy} className="text-gray-500 hover:text-cyan-600 relative">
                            ...
                            return (
                                <React.Fragment key={status.name}>
                                    <div className="flex flex-col items-center text-center w-24">
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isActive ? 'bg-gray-800 text-white' : 'bg-gray-200 text-gray-400'}`}>
                                            <Icon name={status.icon} className="w-6 h-6" />
                                        </div>
                                        <p className={`mt-2 text-sm font-medium ${isActive ? 'text-gray-800' : 'text-gray-500'}`}>{status.name}</p>
                                    </div>
                                    {index < trackingStatuses.length - 1 && (
                                        <div className={`flex-1 h-px ${index < currentStatusIndex ? 'bg-gray-800' : 'bg-transparent border-t-2 border-gray-300 border-dashed'}`}></div>
                                    )}
                                </React.Fragment>
                            );
                        })}
                     ...
                     <div className="relative pl-8">
                         ...
                         {order.trackingHistory.map((event, index) => (
                             <div key={index} className="relative mb-8">
                                 {/* Dot */}
                                 <div className={`absolute -left-5.5 top-1 w-4 h-4 rounded-full ${index === 0 ? 'bg-cyan-500 ring-4 ring-cyan-100' : 'bg-gray-300'}`}></div>
