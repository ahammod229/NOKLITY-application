  ...
  return (
    <header className="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-50 transition-colors duration-200">
      <div className="container mx-auto px-4">
        ...
        <div className="hidden md:flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
          <div></div>
          <div className="flex items-center space-x-2">
            {isAuthenticated && user && (
              <span className="text-gray-700 dark:text-gray-300">Welcome, <b>{user.name}</b></span>
            )}
          </div>
        </div>
        ...
        <div className="md:hidden py-2">
            <div className="flex items-center gap-2">
                <Link to={isAuthenticated ? "/account" : "/login"} className="text-gray-600 dark:text-gray-300 hover:text-noklity-red p-1 flex-shrink-0" aria-label="Account">
                ...
                <div ref={mobileSearchContainerRef} onBlur={handleBlur} className="flex-1 relative">
                    <form onSubmit={handleSearchSubmit} className="flex items-center gap-2">
                        <div className="relative flex-grow">
                          <input
                            ...
                            className="w-full pl-4 pr-10 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 bg-gray-100 dark:bg-gray-700 dark:text-white"
                            autoComplete="off"
                          />
                          {searchQuery && (
                            <button type="button" onClick={handleClearSearch} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white" aria-label="Clear search">
                    ...
                    {showSuggestions && (
                        <div className="absolute top-full left-0 right-0 mt-2 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-20 max-h-96 overflow-y-auto">
                           <ul>
                            {productSuggestions.map(suggestion => {
                                ...
                                <li key={`product-${suggestion.data.id}`}>
                                    ...
                                    className="flex items-center p-3 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200"
                                    >
                                    <img src={suggestion.data.imageUrl} alt={suggestion.data.name} className="w-12 h-12 object-contain mr-4 rounded bg-white" loading="lazy" decoding="async" />
                            ...
                            {productSuggestions.length > 0 && categorySuggestions.length > 0 && (
                                <li className="border-t border-gray-200 dark:border-gray-700 mx-3 my-1"></li>
                            )}
                            {categorySuggestions.map(suggestion => (
                                ...
                                >
                                    ...
                                    <span className="text-xs bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-300 font-medium px-2 py-0.5 rounded-full">Category</span>
                                    </div>
                                </Link>
                                </li>
                            ))}
          ...
          <div ref={desktopSearchContainerRef} onBlur={handleBlur} className="flex-1 max-w-xl mx-8 relative">
            <form onSubmit={handleSearchSubmit} className="relative">
              <input
                ...
                className="w-full px-4 py-2 pr-20 border border-gray-300 dark:border-gray-600 rounded-full focus:outline-none focus:ring-2 focus:ring-noklity-red bg-white dark:bg-gray-700 dark:text-white transition-colors"
                autoComplete="off"
              />
              {searchQuery && (
                  <button type="button" onClick={handleClearSearch} className="absolute right-12 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white" aria-label="Clear search">
            ...
            {showSuggestions && (
              <div className="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-20 max-h-96 overflow-y-auto">
                <ul>
                  {productSuggestions.map(suggestion => {
                    if (suggestion.type !== 'product') return null;
                    return (
            ...
            <ul className="flex items-center space-x-6">
              {navItems.map((item) => (
                <li key={item.label + item.href}>
                  <Link to={item.href} className="flex flex-col items-center text-gray-600 dark:text-gray-300 hover:text-noklity-red dark:hover:text-noklity-red relative">
